...
func (s *Server) Verify(ctx context.Context, input string) (string, error) {
	cmd := exec.Command("cargo", "run", "--bin", "synd-verifier", "--", input)

	out, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("failed to run synd-verifier: %w. Output: %s", err, string(out))
	}

	lines := strings.Split(string(out), "\n")
	mblock := lines[len(lines)-1]

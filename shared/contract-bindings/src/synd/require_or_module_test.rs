...
See the [wrapper's documentation](`RequireOrModuleTestInstance`) for more details.*/
    ...
    >(
        ...
        RequireOrModuleTestInstance::<P, N>::new(address, provider)
    }
...
For more fine-grained control over the deployment process, use [`deploy_builder`] instead.*/
    ...
    > {
        RequireOrModuleTestInstance::<P, N>::deploy(provider)
    }
...
the bytecode concatenated with the constructor's ABI-encoded arguments.*/
    ...
    >(provider: P) -> alloy_contract::RawCallBuilder<P, N> {
        RequireOrModuleTestInstance::<P, N>::deploy_builder(provider)
    }

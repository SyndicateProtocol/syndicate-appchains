...
use crate::{
    ...
    URLErrorType::{InvalidHost, InvalidScheme, InvalidURL},
};
...
pub fn parse_url(value: &str) -> Result<Url, Error> {
    Url::parse(value).map_or_else(
        |_err| Err(Error::URL(InvalidURL(value.to_string()))),
        |url| {
            if !url.has_host() {
                return Err(Error::URL(InvalidHost));
            }
            match url.scheme() {
                "http" | "https" => Ok(url),
                _ => Err(Error::URL(InvalidScheme(url.scheme().to_string()))),
            }
        },
    )
}
...
fn parse_address(value: &str) -> Result<Address, Error> {
    Address::from_str(value).map_err(|_| EthereumAddress(value.to_string()))
}
...
mod tests {
    ...
    fn test_parse_url_invalid_format() {
        ...
        for url in invalid_urls {
            match parse_url(url) {
                Err(Error::URL(InvalidURL(error_url))) => {

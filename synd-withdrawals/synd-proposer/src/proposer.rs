...
impl Proposer {
    ...
    async fn get_l1_start_end_block(
        ...
        let l1_start_block =
            self.fetch_l1_block(tee_trusted_input.l1StartBlockHash, "start").await?;
        let l1_end_block = self.fetch_l1_block(tee_trusted_input.l1EndBlockHash, "end").await?;

        Some((l1_start_block, l1_end_block))
    }

    async fn fetch_l1_block(&self, hash: B256, block_type: &str) -> Option<Block> {
        match self.ethereum_provider.get_block_by_hash(hash).await {
            Ok(Some(block)) => Some(block),
            Ok(None) => {
                error!(%hash, "L1 {} block not found", block_type);
                None
            }
            Err(e) => {
                error!(%e, %hash, "Error fetching L1 {} block", block_type);
                None
            }
        }
    }
    ...
    async fn build_l1_input(
        ...
        let (l1_start_block, l1_end_block) =
            self.get_l1_start_end_block(tee_trusted_input)
                .await
    ...
    async fn get_delayed_messages(
        ...
        match first_log.len() {
            ...
            2.. => {
                panic!(
                    "found multiple blocks for first delayed message: {start_delayed_message_index}",
                )
            }
        }
        #[allow(clippy::unwrap_used)]
...
mod tests {
    ...
    impl Default for Proposer {
        fn default() -> Self {
            ...
            Self {
                ...
                _settlement_provider: dummy_filled_provider,
    ...
    async fn get_latest_contract_txn_block(
        ...
        let block_hash = latest_log.block_hash.ok_or_else(|| eyre!("Block hash not found"))?;
        let tx_hash =
            latest_log.transaction_hash.ok_or_else(|| eyre!("Transaction hash not found"))?;

...
use synd_stake_cli::{
    ...
    proofs::{
        ...
        AggregateGasDataArgs, SubmitGasProofsArgs, UpdateBaseAndEthereumBlockHashesArgs,
    },
...
enum Commands {
    ...
    /// automatically falls back to the offchain aggregation mechanism and submits the gas data for
    /// the top N chains, if we are over the "offchain aggregation" threshold
    AggregateGasData(AggregateGasDataArgs),
}
...
async fn main() {
    ...
    match &args.command.unwrap_or_else(|| panic!("No command provided. use --help for more info")) {
        ...
        Commands::AggregateGasData(args) => aggregate_gas_data_top_n_chains(args).await,
    };
}

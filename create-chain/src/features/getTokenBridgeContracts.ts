...
import { Chain, decodeEventLog, getAbiItem, getEventSelector, Hex, Log, PublicClient, TransactionReceipt, Transport } from "viem";
...
export async function getTokenBridgeContracts({
  ...
  const txReceipt = await parentChainPublicClient.waitForTransactionReceipt({ hash: bridgeCreationHash })

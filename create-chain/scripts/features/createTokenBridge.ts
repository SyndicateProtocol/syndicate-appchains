...
export async function createTokenBridge<TParentChain extends Chain | undefined, TOrbitChain extends Chain | undefined>({
  ...
  setWethGatewayGasOverrides,
    ...
    const isCustomFeeTokenBridge = nativeTokenAddress && nativeTokenAddress !== zeroAddress;
  ...
  const transaction = await parentChainPublicClient.getTransaction({ hash: receipt.transactionHash });

...
import {MetafillerStorage_ReleaseCandidate} from "./backfill/MetafillerStorage_ReleaseCandidate.sol";
import {RequireAllModule_ReleaseCandidate} from "./requirement-modules/RequireAllModule_ReleaseCandidate.sol";
import {RequireAnyModule_ReleaseCandidate} from "./requirement-modules/RequireAnyModule_ReleaseCandidate.sol";
import {IRequirementModule_ReleaseCandidate} from "./interfaces/IRequirementModule_ReleaseCandidate.sol";

/// @title MetabasedFactory
/// @notice Factory contract for creating MetabasedSequencerChain and related contracts
contract MetabasedFactory_ReleaseCandidate {
    ...
    function createMetabasedSequencerChain(
        uint256 l3ChainId,
        address admin,
        IRequirementModule_ReleaseCandidate permissionModule
    ) public returns (address sequencerChain) {
    ...
    function createAllContractsWithRequireAllModule(address admin, address manager, uint256 l3ChainId)
        public
        returns (
            address sequencerChain,
            address metafillerStorage,
            IRequirementModule_ReleaseCandidate permissionModule
        )
    {
        permissionModule = IRequirementModule_ReleaseCandidate(new RequireAllModule_ReleaseCandidate(admin));
    ...
    {
        permissionModule = IRequirementModule_ReleaseCandidate(new RequireAnyModule_ReleaseCandidate(admin));
        (sequencerChain) = createMetabasedSequencerChain(l3ChainId, admin, permissionModule);
        metafillerStorage = createMetafillerStorage(admin, manager, l3ChainId);

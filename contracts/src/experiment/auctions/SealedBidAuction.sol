...
/// @notice A sealed bid auction contract. A sealed-bid auction is a type of auction process in which all bidders simultaneously submit sealed bids to the auctioneer so that no bidder knows how much the other auction participants have bid.
contract SealedBidAuction is IAuction {
    struct Bid {
        bytes32 sealedBid;
        uint256 deposit;
    }
    ...
    function withdrawFunds() external override {
        ...
        if (refund == 0 || !auctionActive) revert NoFundsToWithdraw();

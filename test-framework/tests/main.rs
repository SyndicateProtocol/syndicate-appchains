...
// For standalone files in the `/tests` root, `cargo` compiles EACH test file into a separate
// binary. We prefer one `main.rs` test file with one binary for a faster build.
// More detail:

...
// Test L2 sequencing chain crash and restart scenario
#[tokio::test]
async fn e2e_maestro_l2_crash_recovery() -> Result<(), eyre::Error> {
    TestComponents::run(
        &ConfigurationOptions { use_write_loop: true, ..Default::default() },
        |components| async move {
            components.sequencing_provider.anvil_set_block_timestamp_interval(0).await?;
            ...
            // 1. Send first transaction through Maestro -> batch-sequencer -> L2
            ...
            // Give the batch sequencer time to process but not complete the transaction
            tokio::time::sleep(Duration::from_millis(500)).await;

            // 4. At this point:
            //    - tx1 should be confirmed on appchain
            //    - tx2 should be in Maestro's pending transactions but may not be on L2 yet
            //    - batch-sequencer should lose connection to L2
            ...
            // - Wait for batch-sequencer to reconnect
            // - Verify that pending transactions are resubmitted

            // 6. For this test, let's verify the system can recover by checking transaction state
            // The batch-sequencer should eventually resubmit tx2 when connectivity is restored

            println!("Test completed - demonstrated L2 crash scenario handling");

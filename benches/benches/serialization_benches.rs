...
fn arbitrum_batch_serialization(scenario: LoadScenario) {
    ...
    let batch =
        ArbitrumBatch::new(bench_utils::generate_random_bytes(batch_size * 200), delayed_messages);
...
#[divan::bench(args = [10, 50, 100, 500])]
fn block_serialization(num_messages: usize) {
    let mut rng = rand::rng();
    use rand::Rng;
...
fn serialization_memory_usage(bencher: divan::Bencher<'_, '_>, batch_size: usize) {
    let mblock = create_test_mblock(batch_size, true);
...
fn roundtrip_serialization(batch_size: usize) {
    let original = create_test_mblock(batch_size, true);
...
fn delayed_message_batch_serialization(num_messages: usize) {
    let mut rng = rand::rng();
    use rand::Rng;

    let messages: Vec<DelayedMessage> = (0..num_messages)
        .map(|_| DelayedMessage {
            kind: rng.random_range(0..10),
            sender: generate_random_address(),

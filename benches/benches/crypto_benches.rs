...
fn keccak256_hashing_single(input_size: usize) {
    let data = bench_utils::generate_random_bytes(input_size);
    divan::black_box(keccak256(&data));
}

/// Benchmark batch keccak256 hashing (simulating accumulator updates)
#[divan::bench(args = [1, 10, 50, 100, 500, 1000])]
fn keccak256_batch_hashing(num_hashes: usize) {
    let data_sets = generate_hash_data(num_hashes);
...
fn accumulator_hashing_chain(chain_length: usize) {
    let mut acc = FixedBytes::ZERO;
    let message_hash = generate_random_hash();
...
fn complex_message_hashing(num_messages: usize) {
...
fn signature_verification_batch(num_signatures: usize) {
    // Generate realistic transaction data for signature verification
...
#[divan::bench(args = [1, 10, 50, 100])]
fn transaction_validation_pipeline(num_transactions: usize) {
    // Use a valid transaction for benchmarking
    let tx_data = "0xf86d8202b28477359400825208944592d8f8d7b001e72cb26a73e4fa1806a51ac79d880de0b6b3a7640000802ca05924bde7ef10aa88db9c66dd4f5fb16b46dff2319b9968be983118b57bb50562a001b24b31010004f13d9a26b320845257a6cfc2bf819a3d55e3fc86263c5f0772";
    let tx_bytes = alloy::primitives::hex::decode(tx_data).expect("valid hex");
    let raw_tx = alloy::primitives::Bytes::from(tx_bytes);
...
fn address_recovery_performance(num_recoveries: usize) {
    let tx_data = "0xf86d8202b28477359400825208944592d8f8d7b001e72cb26a73e4fa1806a51ac79d880de0b6b3a7640000802ca05924bde7ef10aa88db9c66dd4f5fb16b46dff2319b9968be983118b57bb50562a001b24b31010004f13d9a26b320845257a6cfc2bf819a3d55e3fc86263c5f0772";
    let tx_bytes = alloy::primitives::hex::decode(tx_data).expect("valid hex");
    let raw_tx = alloy::primitives::Bytes::from(tx_bytes);
    let tx = decode_transaction(&raw_tx).expect("valid transaction");
...
#[divan::bench(args = [10, 50, 100, 500])]
fn abi_encoding_performance(num_encodings: usize) {
    let sender = generate_random_address();
    let seq_block_number = 12345u64;
    let timestamp = 1700000000u64;
    let base_fee_l1 = U256::from(1000000000u64);
...
fn crypto_memory_allocation(bencher: divan::Bencher<'_, '_>, num_operations: usize) {

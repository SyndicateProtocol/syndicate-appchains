...
use debug_translator::{config::MetabasedConfig, debug::Debugger};
use eyre::Result;
...
async fn run(
    ...
    let (debugger_shutdown_tx, debugger_shutdown_rx) = oneshot::channel();
    ...
    // Create debug channel for sequencing and settlement
    let (debug_settlement_tx, debug_settlement_rx) = mpsc::channel(100);
    let (debug_sequencing_tx, debug_sequencing_rx) = mpsc::channel(100);
    ...
    // Create debug channel for block builder
    let (debug_slot_tx, debug_slot_rx) = mpsc::channel(100);

    let block_builder = BlockBuilder::new(debug_slot_rx, block_builder_config).await?;
    ...
    let debugger_handle = tokio::spawn(async move { debugger.run(debugger_shutdown_rx).await });
    ...
    tokio::select! {
        _ = main_shutdown_rx => {
            ...
            tokio::time::timeout(std::time::Duration::from_secs(5 * 60), async {
                ...
                let _ = debugger_shutdown_tx.send(());
                let _ = debugger_handle.await;

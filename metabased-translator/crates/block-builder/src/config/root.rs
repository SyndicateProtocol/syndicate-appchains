...
// TODO remove me
#[derive(Debug, Deserialize, IntoStringKVPairs)]
...
mod tests {
    ...
    const DUMMY_VALUES: [(&str, &str); 1] = [("sequencer_port", "101")];
    ...
    // Testing the IntoStringKVPairs implementation
    #[test]
    fn test_configuration_iterator_values() {
        let config = Configuration::default();
        let values: Vec<(String, String)> = config.into_iter().collect();

        assert_eq!(
            values,
            vec![
                ("port".to_string(), "8888".to_string()),
                ("genesis_timestamp".to_string(), "1712500000".to_string()),
                ("chain_id".to_string(), "84532".to_string()),
            ]
        );
    }
    ...
    // Testing the IntoStringKVPairs implementation
    #[test]
    fn test_configuration_iterator_order() {
        let config = Configuration::default();
        let values: Vec<String> = config.into_iter().map(|(key, _)| key).collect();

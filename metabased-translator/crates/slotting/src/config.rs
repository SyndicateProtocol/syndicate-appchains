...
mod config_tests {
    ...
    fn test_new_with_validation() {
        // Valid config
        let result = SlotterConfig::new(2_000, 0);
        assert!(result.is_ok());

        // Invalid config
        let result = SlotterConfig::new(0, 0);
    ...
    fn test_validation() {
        // Test zero duration
        let result = SlotterConfig::new(0, 1_000_000);
        assert_matches!(
            result.unwrap_err(),
        ...
        let result = SlotterConfig::new(2_000, 1_000_000);
        assert!(result.is_ok());
    }
}

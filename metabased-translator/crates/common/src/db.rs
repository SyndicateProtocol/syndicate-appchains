...
pub trait TranslatorStore {
    ...
    /// returns the latest safe state known (latest safe slot, and last considered block from each chain)
    async fn get_latest(&self) -> Result<Option<(Slot, Option<Block>, Option<Block>)>, DbError>;
}
...
impl TranslatorStore for RocksDbStore {
    async fn save_slot(&self, slot: &Slot) -> Result<(), DbError> {
        let serialized = bincode::serialize(slot)?;
        let mut batch = rocksdb::WriteBatch::default();
        batch.put(Self::slot_key(slot.slot_number), serialized);
        batch.put(
            KEY_SEQ_LATEST,
    ...
    async fn get_latest(&self) -> Result<Option<(Slot, Option<Block>, Option<Block>)>, DbError> {
        ...
        match iter.last().transpose()? {
            ...
            Some((_, v)) => {
                ...
                let settle = self
                    .db
                    .get(KEY_SETTLE_LATEST)?
                    .map(|v| bincode::deserialize(&v))
                    .transpose()?;
                Ok(Some((slot, seq, settle)))
            }
        }
    }
}
...
pub fn test_path() -> String {
    ...
    std::env::temp_dir()
        .join(format!("rocksdb_test_{:x}", hash))
        .to_str()
        .unwrap()
        .to_string()
}

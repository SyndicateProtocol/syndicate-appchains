use crate::contract_bindings::translator_contracts::eventemitter::EventEmitter;
...
pub async fn run() -> eyre::Result<()> {
    ...
    provider
        .anvil_set_code(address, bytecode.clone().into())
        .await?;
...
async fn deploy_contracts(server_url: String) -> Result<(), Report> {
    ...
    for address in addresses.iter() {
        anvil_provider
            .anvil_set_code(*address, event_emitter_bytecode.clone())
            .await?;
    }
    Ok(())
}
...
mod tests {
    use super::*;
    use crate::contract_bindings::translator_contracts::eventemitter::EventEmitter::EventEmitterInstance;
    ...
    async fn test_deploy_event_emitter_contracts() -> eyre::Result<()> {
        ...
        let tx = contract
            .emitEvent1(sig_hash, non_indexed.into())
            .gas(1_000_000)
            .nonce(0)

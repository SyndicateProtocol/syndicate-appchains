...
*This function can only be called by addresses with EMISSION_MINTER_ROLE.
...
*Only callable by addresses with AIRDROP_MANAGER_ROLE and only during lock period*
...
|Name|Type|Description|
|----|----|-----------|
|`<none>`|`uint256`|The amount of tokens still available to mint to reach TOTAL_SUPPLY|
...
*Implements transfer restrictions based on lock timestamp and roles*

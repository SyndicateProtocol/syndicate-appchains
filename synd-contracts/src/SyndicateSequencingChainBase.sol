...
/// @title SyndicateSequencingChainBase
/// @notice Abstract base contract for transaction sequencing using Syndicate's "secure by module design" architecture
///
...
/// 5. If allowed, _transactionProcessed is called (can be overridden by derived contracts)
...
abstract contract SyndicateSequencingChainBase is SequencingModuleChecker, ISyndicateSequencingChain, GasCounter {
    error NoTxData();
    ...
    /// @notice Constructs the SyndicateSequencingChainBase contract.
    ...
    function processTransaction(bytes calldata data) external trackGasUsage {
        ...
        _transactionProcessed(transaction);
    }
    ...
    /// @notice Hook called when a transaction is processed
    /// @dev Override this in derived contracts to add custom behavior (e.g., accumulator updates)
    /// @param transaction The encoded transaction data
    function _transactionProcessed(bytes memory transaction) internal virtual {
